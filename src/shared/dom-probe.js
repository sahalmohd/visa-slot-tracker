/**
 * Injected into the target checkvisaslots.com tab/page. Self-contained —
 * no imports, no closures over outer module scope — so it can run
 * verbatim in two different environments:
 *   - the extension, via chrome.scripting.executeScript({ func: domProbe })
 *   - the headless CI checker, via Playwright's page.evaluate(domProbe, label)
 *
 * Strategy:
 ...
 */
export async function domProbe(targetMonthLabel) {
